...
private:
    ...
    ros::Subscriber joy_sub_reverse;
...
public:
    Mux() {
	    ...
	    //joy_sub_reverse = n.subscribe(joy_topic, 1, &Mux::joy_callback_reverse, this);
    ...
    void joy_callback(const sensor_msgs::Joy & msg) {
        ...
        //std::cout<<"reverse: "<<mux_controller[joy_mux_reverse_idx]<<std::endl;
        std::cout<<"reverse: "<<mux_controller[joy_mux_reverse_idx]<<std::endl;
        if (mux_controller[joy_mux_idx]) 
	    {
		    int output_joy = msg.axes[joy_speed_axis];
 			std::cout<<"msg.axes[joy_speed_axis]: "<<msg.axes[joy_speed_axis]<<std::endl;
 			std::cout<<"joy_speed_axis: "<<joy_speed_axis<<std::endl;
            if(output_joy<0)
            {
                output_joy = 0;
            }
                ...
                double desired_velocity = max_speed * output_joy;
                double desired_steer = max_steering_angle * msg.axes[joy_angle_axis];

		std::cout<<"going forward"<<std::endl;

        //PUBLISH message from wall following to this joysticki /steering wheel

                publish_to_drive(desired_velocity, desired_steer);
        }

        else if(mux_controller[joy_mux_reverse_idx])
        {
		    ...
		    // Calculate desired velocity and steering angle
		    double desired_velocity = max_speed * -1*output_joy;
		    double desired_steer = max_steering_angle * msg.axes[joy_angle_axis];
	...
	// void joy_callback_reverse(const sensor_msgs::Joy & msg) {
	// 	// make drive message from joystick if turned on
	// 	if (mux_controller[joy_mux_idx]) 
	// 	{
	// 		int output_joy = msg.axes[joy_speed_axis];
