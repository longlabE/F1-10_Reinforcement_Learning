...
#include "f1tenth_simulator/detected_drowsy.h"
...
private:
    ...
    // Mux indices
    int joy_mux_idx;
    int joy_mux_reverse_idx;
    int key_mux_idx;
    int nav_mux_idx;
    int brake_mux_idx;
    int autonomous_park_mux_idx;
    ...
    int nav_button_idx;
    int autonomous_park_button_idx;
    ...
    std::string autonomous_park_key_char;
...
public:
    BehaviorController() {
        ...
        drowsy_sub = n2.subscribe("drowsy_topic", 1000, &BehaviorController::drowsyCallback, this);
        ...
        n.getParam("joy_mux_reverse_idx", joy_mux_reverse_idx);
        n.getParam("key_mux_idx", key_mux_idx);
        n.getParam("brake_mux_idx", brake_mux_idx);
        n.getParam("nav_mux_idx", nav_mux_idx);
	    n.getParam("autonomous_park_mux_idx", autonomous_park_mux_idx);
        // ***Add mux index for new planner here***
	...
	n.getParam("autonomous_park_button_idx", autonomous_park_button_idx);
        // ***Add button index for new planner here***
	...
	n.getParam("autonomous_park_key_char", autonomous_park_key_char);
        // ***Add key char for new planner here***
	...
	{
            // nav
            toggle_mux(joy_mux_reverse_idx, "Reverse");
        }

	else if (msg.buttons[autonomous_park_button_idx]) 
	{
            // nav
            toggle_mux(autonomous_park_mux_idx, "Autonomous Park");
        }
	...
	else if(msg.data == autonomous_park_key_char)
	{
	//autonomous_park
    ...
    void drowsyCallback(const f1tenth_simulator::detected_drowsy::ConstPtr& msg)
    {
        //ROS_INFO("Switch to Autonomous:  %d ", msg->detected_msg);

        if(msg->detected_msg)
        {
            // change to autonomous park mode
            change_controller(2);
